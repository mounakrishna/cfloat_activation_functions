...
TOPFILE   ?= $(DISTRO)/test/tb_cfloat.bsv
...
BSC_PATHS = -p $(DISTRO)/src:$(DISTRO)/test:%/Prelude:%/Libraries
...
compile: build_bsim
	...
	bsc -u -sim  $(BSIM_DIRS) $(BSC_COMP_FLAGS) $(MOREDEFINES) $(BSC_PATHS) -g $(TOPMODULE)  $(TOPFILE) 
...
verilog: build_v  verilog_dir
	...
	@echo $(MOREDEFINES)
	bsc -u -verilog $(V_DIRS) $(BSC_COMP_FLAGS) $(MOREDEFINES) $(BSC_PATHS) -g $(TOPMODULE)  $(TOPFILE)
